...
Lightweight Bash tool to analyze plain-text logs quickly: count errors, extract top IPv4s, or grep by regex. Designed to meet common scripting-course requirements (purpose, README, options/args with -h, regex, file I/O, error handling, output changes with flags).
...
It uses only standard Unix tools and is suitable for quick troubleshooting and coursework.
...
IPv4 regex: ([0-9]{1,3}\.){3}[0-9]{1,3} (fast and practical; may include private/broadcast ranges).
...
MIT is recommended. Add a LICENSE file or replace this with your chosen license text.
